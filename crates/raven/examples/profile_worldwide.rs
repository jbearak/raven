...
fn main() {
    ...
    for _ in 0..3 {
        let t = Instant::now();
        let (index, imports, cf, new_idx, _, _, _) = scan_workspace(&[folder.clone()], 20);
    ...
    for _ in 0..3 {
        ...
        let (index, imports, cf, new_idx, pkg_ws, pkg_ns, _) = scan_workspace(&[folder.clone()], 20);
        let mut state = make_state(&workspace);
        let t = Instant::now();
        state.apply_workspace_index(index, imports, cf, new_idx, pkg_ws, pkg_ns);
