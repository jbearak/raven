...
- **R Markdown / Quarto preview** — REditorSupport offers an `.Rmd` preview in a webview panel (with an opt-in auto-refresh setting); the [`quarto.quarto`](https://marketplace.visualstudio.com/items?itemName=quarto.quarto) extension offers `Quarto: Preview` for `.qmd` (render-on-save is gated by the `quarto.render.renderOnSave` setting). Raven's `Raven: Knit Preview` is intentionally a static viewer with a manual **Knit again** button — `Shift+Cmd+Enter` (or `Shift+Ctrl+Enter`) saves the file (if unsaved) and re-knits in one keystroke, so re-rendering stays explicit. See [Knit](./knit.md) for the design rationale. `.qmd` rendering itself is the Quarto extension's domain regardless.
