...
- **Live R Markdown / Quarto preview that auto-refreshes** — REditorSupport renders `.Rmd` previews in a webview panel that updates on save; the `quarto.quarto` extension does the same for `.qmd`. Raven supports chunk highlighting, chunk navigation, CodeLens, and a static `Raven: Knit Preview` (see [Chunks](./chunks.md) and [Knit](./knit.md)) with a manual refresh button, but no auto-refreshing preview and no `.qmd` rendering.
