...
- **Auto-refreshing R Markdown / Quarto preview** — REditorSupport's `.Rmd` preview and the [`quarto.quarto`](https://marketplace.visualstudio.com/items?itemName=quarto.quarto) extension's `.qmd` preview both auto-refresh on save. Raven's `Raven: Knit Preview` is intentionally a static viewer with a manual **Knit again** button — `Shift+Cmd+Enter` (or `Shift+Ctrl+Enter`) saves the file (if unsaved) and re-knits in one keystroke, so re-rendering stays explicit. See [Knit](./knit.md) for the design rationale. `.qmd` rendering itself is the Quarto extension's domain regardless; if you specifically want background re-rendering on save, REditorSupport or Quarto provide it today.
