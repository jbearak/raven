...
- **Live R Markdown / Quarto preview that auto-refreshes** — REditorSupport (with the R Markdown extension) and the Quarto extension render `.Rmd`/`.qmd` previews in a webview panel that updates on save. Raven supports chunk highlighting, chunk navigation, CodeLens, and a static `Raven: Knit Preview` (see [Chunks](./chunks.md) and [Knit](./knit.md)) with a manual refresh button, but no auto-refreshing preview and no `.qmd` rendering.
