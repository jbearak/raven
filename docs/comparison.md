...
REditorSupport doesn't provide a command for sending code to an arbitrary terminal — only to its own managed R session. If you need to send code to R running inside `tmux` (so the session survives VS Code restarts), you have to fall back on VS Code keybindings like `workbench.action.terminal.sendSequence`. Those keybindings only send the current line (no multi-line statement detection), and multi-line pastes via that mechanism can arrive garbled.

Raven's `sendMethod` (default `auto`) pastes short blocks directly and switches to a temp-file `source()` once a block reaches `raven.sendToR.autoTempFileThresholdLines` (default 25). The temp-file path bypasses the typing-simulation pipe entirely: R reads the file from disk, so for those larger blocks there's no inter-line delay. Raven also exposes a separate **Terminal** submenu of send commands in the editor toolbar (and matching commands in the Command Palette) that targets whatever terminal is currently active in VS Code — for example, an `R` running inside `tmux` or a Docker container — using the same statement detection and temp-file fallback. See [R Console: Editor Toolbar](./r-console.md#editor-toolbar) and [Send method](./r-console.md#send-method) for details.
