...
A latest-CRAN/Bioc export database — the R-free floor for broad ecosystem coverage when the sidecar is installed. Coverage for R's base-priority packages is **embedded directly in the binary** — no sidecar needed for them. Raven embeds all 14 base-priority packages, but only the default-attached **Base-7** (`base`, `methods`, `utils`, `grDevices`, `graphics`, `stats`, `datasets`) are always in scope; the other 7 (compiler, grid, parallel, splines, stats4, tcltk, tools) resolve from the embedded table only after an explicit `library()` call. `names.db`'s post-merge filter strips the default-attached **base-7**, so it never re-supplies those always-in-scope symbols; it covers CRAN, Bioconductor, and recommended packages. (The other 7 base-priority packages may still appear in it from a reference-R capture, but the embedded cache preempts them at runtime, so the copies are harmlessly redundant.) Release archives, VSIX installs, and package-manager builds ship `names.db` next to the Raven executable; the floor is one sidecar. Cargo/source installs through `cargo install --git https://github.com/jbearak/raven raven` install only the executable; run `raven packages update` once in the user account or CI image to download the mutable user-data copy.
