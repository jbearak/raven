...
1. **Determine effective workspace.** Resolution requires *parsing* `Package:` from `description.text`, not merely observing that DESCRIPTION exists. Decision table:
   ...
   > **Decision (2026-06-17) — single-signal `Auto` activation is deliberate; do not re-add a NAMESPACE heuristic.** `Auto` activates on exactly one signal: a workspace-root DESCRIPTION with a non-empty `Package:` field — R's own definition of a package. NAMESPACE / roxygen / `R/` presence is **not** an activation heuristic. A NAMESPACE without a `Package:` field is meaningless to R itself (§11.1), and broadening activation biases toward the *unsound* direction — wrongly suppressing real undefined-variable diagnostics in non-packages. A more lenient "any package marker activates" rule was considered and rejected (fuzzier to explain, more false activations); non-standard layouts opt in via `Enabled`.
   >
   > **History:** the `effective_workspace` helper briefly carried a `has_namespace_and_sources` branch (added in the 2026-06-07 "package corpus hardening" commit) that activated `Auto` on NAMESPACE + ≥1 R source with name `"unknown"`. That was an undocumented drift from this section and was reverted 2026-06-17, restoring the rule as written here. Reviewers: this is the intended behavior — a diff that re-introduces NAMESPACE-based `Auto` activation is a regression, not a fix.
