...
package set derived from the table must match `get_base_priority_packages()`.
...
It captures all 14 base-priority packages from a reference R and writes the generated `.rs`,
...
- **Content (M1, comprehensive):** all installed packages (minus all 14 base-priority packages,
...
Raven embeds the 14 base-priority packages' exports as a generated Rust source
...
all base-priority packages and carries only the broader ecosystem. Reversible but with real cost (it
...
(the maintainer's full installed library minus all 14 base-priority packages, ∪ CRAN/Bioc) committed to
