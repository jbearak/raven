...
2. **Tier 3 source & storage (§8):** the shipped `names.db` is built from **the build machine's entire installed library (authoritative reference-R capture) ∪ CRAN + Bioc r-universe, append-only** — *not* r-universe alone. The full installed library is the point: hard-to-install / GitHub-only / internal packages enter the floor this way and append-only never drops them. **Merge is version-driven and monotonic** (revised after PR review — see the plan's decision #16): for each package the merge keeps the record with the **highest version across all three sources** (prior database, reference-R install, r-universe), so a package is **never moved to an older export set** — a newer CRAN/Bioc release overrides an older reference-R install, a newer local/dev build overrides an older CRAN release, and a still-newer version already in the prior database is retained. Equal versions tiebreak reference-R → r-universe → prior. Nothing is ever dropped. Each record therefore carries a package **version** (in both encodings). Stored on a **GitHub Release** (`names-db` tag), bootstrapped once from a richly-provisioned machine; bundled exe-relative with the binary/VSIX. r-universe is fetched from both `cran.r-universe.dev` and `bioc.r-universe.dev`.
...
5. **Shipped database is sourced from an authoritative reference-R capture ∪ r-universe `_exports`/`_dependencies`/`_datasets`, append-only** (revised — see §0.2; the original draft said r-universe alone). The append merge is **version-driven and monotonic**: when sources disagree, the highest version across all sources wins, so a package is never regressed to an older export set (see §0.2).
...
**Why generate Tier 2 when Tier 3 already ships a floor?** Framing matters in the docs: a user gets package-aware diagnostics in CI **regardless** — the bundled Tier 3 database is always there. Tier 2 is not "how to turn diagnostics on"; it is how to make them *more accurate* in two specific cases: (1) the repo uses packages whose exports **aren't shipped** in Raven's Tier 3 database (GitHub-only / internal / not-yet-indexed), and (2) the repo pins package versions whose exports **differ** from the versions Tier 3 captured, in ways that could change diagnostics (the §13 latest-only drift). Absent both, Tier 3 alone suffices and `freeze` is unnecessary.
...
- **Source (revised — see §0.2):** an **authoritative reference-R capture of the build machine's entire installed library** ∪ **CRAN + Bioc r-universe**, merged **append-only**. The reference-R capture (via Raven's Tier-1 path: `NAMESPACE` + subprocess `exportPattern` expansion + `data/` datasets) covers base/recommended *and* every installed package — including hard-to-install / GitHub-only / internal ones that r-universe lacks. r-universe (`/api/ls` + per-package `_exports`/`_dependencies`/`_datasets`, from both `cran.r-universe.dev` and `bioc.r-universe.dev`) supplies packages not in the reference-R library. **The merge is version-driven and monotonic** (revised after PR review — see the plan's decision #16): for each package the build keeps the record with the **highest version across all three sources** (prior database, reference-R install, r-universe), so a package is **never moved to an older export set** — a newer CRAN/Bioc release overrides an older reference-R install, a newer local/dev build overrides an older CRAN release, and a still-newer version already in the prior database is retained. Equal versions tiebreak reference-R → r-universe → prior. Rebuilds never drop a package. Each record carries a package **version** for this comparison, captured from `DESCRIPTION` `Version` (reference-R / `freeze`) or the r-universe JSON `Version`; see the plan's Tasks 1.2/4.1–4.2.
...
- **Freshness:** equals Raven's release/refresh cadence — a periodic refresh plus a rebuild on each release. **The exact refresh interval is not yet committed** (deliberately left open for now): the build supports on-demand (`workflow_dispatch`) and on-release rebuilds, with any scheduled interval to be decided later. Acceptable because the database tracks latest CRAN/Bioc by design and export names are stable; append-only means coverage only grows.
...
`raven check --report-uninstalled` (name adjustable) re-enables missing-package warnings in CI — useful when a pipeline *does* install packages (e.g. `renv::restore()`) and wants to catch any that failed. It is also the right choice for any CI workflow that **actually runs the project's R scripts** after `raven check` (e.g. R-package CI that proceeds to execute code, or any pipeline where a `library()` call must really succeed at runtime): there, an uninstalled package is a genuine failure the user wants surfaced, and the export database — which only suppresses undefined-variable noise — must not mask it. Document this prominently so social-science CI (gate-only, never runs the scripts) and software/R-package CI (runs the scripts) each pick the right mode. Its documentation must state precisely that it reports `library()` calls **not present in the local library paths** — *not* relative to Tier 2/Tier 3 metadata.

> **Decision (revised after PR review):** we considered splitting CI into a separate `raven ci` command that suppresses missing-package while `raven check` reported it by default (so a local `raven check` always nags). On reflection this was **rejected** — it inverts the long-standing CI convention (`check` is the CI verb) and adds a second command for one default. `raven check` keeps suppressing missing-package by default; `--report-uninstalled` is the single opt-in. See the plan's decision #17.
...
- **Tier 3 tracks the latest CRAN/Bioc as of Raven's last database refresh.** Two soft drifts follow. (a) A just-removed export may linger until the next rebuild. (b) If your project uses a **newer** version of a package than the one Tier 3 captured when Raven last refreshed its database, a symbol added in that newer version is unknown to Tier 3 and can surface as a **false-positive undefined-variable** diagnostic. Both are rare and soft; a project that needs exactness pins it via Tier 2 (`raven packages freeze`), which reads exports from the version actually installed. (Avoid the bare jargon "latest-only" in user docs — describe this concretely.)
...
- **Build-job hosting & cadence** → GitHub Actions `build-names-db.yml` publishing to the `names-db` GitHub Release; `workflow_dispatch` + on release, with any scheduled interval **not yet committed** (revised after PR review); append-only (§0.2).
