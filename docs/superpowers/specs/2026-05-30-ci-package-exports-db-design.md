...
2. **Tier 3 source & storage (§8):** the shipped `names.db` is built from **the build machine's entire installed library (authoritative reference-R capture) ∪ CRAN + Bioc r-universe, append-only** — *not* r-universe alone. The full installed library is the point: hard-to-install / GitHub-only / internal packages enter the floor this way and append-only never drops them. **Merge precedence is version-aware** (revised after PR review — see the plan's decision #16): among the sources that carry a package in the current build, the **newest package version's** export set wins — so a newer CRAN/Bioc release in r-universe overrides an older reference-R install (and vice-versa); equal versions prefer the reference-R capture (authoritative, post-load); the prior database is the append-only floor, retained only for packages absent from both current sources. Stored on a **GitHub Release** (`names-db` tag), bootstrapped once from a richly-provisioned machine; bundled exe-relative with the binary/VSIX. r-universe is fetched from both `cran.r-universe.dev` and `bioc.r-universe.dev`.
...
5. **Shipped database is sourced from an authoritative reference-R capture ∪ r-universe `_exports`/`_dependencies`/`_datasets`, append-only** (revised — see §0.2; the original draft said r-universe alone). The append merge is **version-aware**: when sources disagree, the newest package version's exports win (see §0.2).
...
- **Source (revised — see §0.2):** an **authoritative reference-R capture of the build machine's entire installed library** ∪ **CRAN + Bioc r-universe**, merged **append-only**. The reference-R capture (via Raven's Tier-1 path: `NAMESPACE` + subprocess `exportPattern` expansion + `data/` datasets) covers base/recommended *and* every installed package — including hard-to-install / GitHub-only / internal ones that r-universe lacks. r-universe (`/api/ls` + per-package `_exports`/`_dependencies`/`_datasets`, from both `cran.r-universe.dev` and `bioc.r-universe.dev`) supplies packages not in the reference-R library. **Merge precedence is version-aware** (revised after PR review — see the plan's decision #16): for a package carried by more than one current source, the record with the **highest package version** wins, so a newer CRAN/Bioc release overrides an older reference-R install (and vice-versa); equal versions prefer the reference-R capture (post-load truth, and it alone covers non-CRAN packages). The prior database is the append-only floor — its record is retained only when neither current source has the package. Rebuilds never drop a package. (This requires the build to carry each source's package **version**; see the plan's Tasks 4.1–4.2.)
