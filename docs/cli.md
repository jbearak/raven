...
The [`jbearak/setup-raven`](https://github.com/jbearak/setup-raven) action installs Raven from the prebuilt GitHub Release binary for the runner platform, verifies the published SHA-256 checksum, and adds `raven` to `PATH`. It installs only: every `raven` command — `packages update`/`fetch`, `check`, `lint` — is an explicit `run:` step your workflow controls. Normal CI users need neither Rust nor Cargo. It supports Linux and macOS runners on x64 and arm64. Windows runners are not a v1 target — but that is a limitation of the *action* (CI runner support), not of Raven: Raven itself runs on Windows and publishes Windows release binaries, so you can install it on a Windows runner by hand. There is rarely a reason to, though — Raven's analysis is platform-independent, so a Linux runner produces identical results and costs less (GitHub bills Windows runners at a higher minute rate). Set `version` to a specific release tag for fully reproducible CLI versions; the example tracks the latest release.
