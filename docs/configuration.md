...
| `raven.crossFile.backwardDependencies` | "auto" | How backward dependencies are resolved: `"auto"` infers from workspace scan, `"explicit"` uses only `@lsp-sourced-by` directives (disables workspace-scan inference) (see [Backward Dependency Modes](cross-file.md#backward-dependency-modes)) |
