...
- **You'd benefit** — e.g., you send large blocks to R over `tmux` / SSH (Raven's temp-file `source()` sidesteps bracketed-paste issues), or you regularly view multi-million-row frames (Raven's data viewer streams Arrow rows on demand). See [Comparison: R session integration](./comparison.md#r-session-integration).
...
These last two cases are why `raven.rConsole.activation` defaults to `auto`, which steps Raven's R-session features aside whenever REditorSupport is enabled. Raven's language server and help viewer activate either way — you don't lose cross-file intelligence by leaving the R console off.
