...
Raven contributes dedicated `rmd` and `quarto` language IDs for `.Rmd` and `.qmd` files. Plain-R snippets from `r.json` (`for` / `fun` / `if` etc.) also expand inside `.Rmd` / `.qmd` buffers when Raven's R console is active — typically what you want when the cursor sits inside an R chunk. These overlap directly with REditorSupport's `r-snippets.json` triggers (`if`, `for`, etc.), so they are gated behind `raven.rConsole.activation`; see [Coexistence](./coexistence.md). The R Markdown and Quarto chunk / helper snippets below use distinct, compact prefixes (`rchunk`, `setupchunk`, etc.) — REditorSupport ships its own R Markdown snippets, but they use different, longer-form prefixes (`r code chunk`, `inline r code`, etc.), so the two sets coexist without trigger collisions and Raven's register unconditionally.
