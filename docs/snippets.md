...
Raven contributes dedicated `rmd` and `quarto` language IDs for `.Rmd` and `.qmd` files. Plain-R snippets from `r.json` (`for` / `fun` / `if` etc.) are registered unconditionally for `.R` files via `package.json` — no console activation required. Inside `.Rmd` / `.qmd` buffers the same snippets are also made available, but only when Raven's R console is active, because they overlap directly with REditorSupport's `r-snippets.json` triggers (`if`, `for`, etc.) inside fenced R chunks. That gating is controlled by `raven.rConsole.activation`; see [Coexistence](./coexistence.md). The R Markdown and Quarto chunk / helper snippets below use distinct, compact prefixes (`rchunk`, `setupchunk`, etc.) — REditorSupport ships its own R Markdown snippets, but they use different, longer-form prefixes (`r code chunk`, `inline r code`, etc.), so the two sets coexist without trigger collisions and Raven's register unconditionally.
