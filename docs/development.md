...
files. Seeding only from resident documents loses helper and package definitions
...
- `declared_nse_policy(pkg, name)` and `declared_bracket_nse_packages()` read the per-package `raven/nse.toml` declarations (`PackageInfo::nse_declarations`) from the `packages` ArcSwap snapshot. Both are synchronous and cached-only: they run inside the undefined-variable collector (`package_arg_policy` / `BracketNsePackages::assemble` in `handlers.rs`), which must never block on a package load, so an uncached declaring package simply contributes nothing until the normal load path warms it. `BracketNsePackages::assemble` runs once per `NseAnalysis::build` and merges the declared `[subset]` entries under Raven's built-in `data.table` entry (built-in wins on collision, and the built-in data.frame-family / `setDF` / `setattr` names are checked before any declared list). Each declared entry is marked bare-callable only when its package is in `in_play_packages`, named by a `pkg::` qualifier in the file (`collect_namespace_qualifiers`, a pre-scan run before the fact walk), or is the `self_nse_package`; the built-in `fread`/`setDT` names classify unconditionally.
