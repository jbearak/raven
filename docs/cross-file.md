...
3. **Tier 3 — sidecar database.** Raven's `names.db` sidecar, built **append-only** from a reference-R capture ∪ CRAN + Bioconductor (via [r-universe](https://r-universe.dev)), keeping the **highest version** of each package. Release archives and package-manager builds ship it next to the executable; source/Cargo installs need `raven packages update` for broad CRAN/Bioconductor coverage. Carries exports, `Depends`, and dataset names — no `:::` internals or signatures.
