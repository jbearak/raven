...
named arguments from the allowlisted constructors above and visible
`foo$bar <- ...` assignments are offered as field completions. When a partial
...
package-data introspection, non-bare left-hand sides, `[[`/`[` completions, and
chained access (`foo$bar$baz` returns nothing).
