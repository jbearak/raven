...
| `box-module-not-found` | A static relative or configured `box::use()` target does not resolve to a file module or `__init__.r` / `__init__.R` module | No |
| `box-module-case-mismatch` | A static relative or configured box module target exists only under a different filename case | No |
...
- Inside inline R6 methods (including defaults and nested closures), `self`, `private`, and `super` are in scope. Classes with literal `portable = FALSE` also expose bare public, private, and active members, including statically resolved inheritance. These bindings do not leak into field initializers or unrelated functions. Known masks disable bare `R6Class`/`list` recognition; `R6::R6Class` and `base::list` retain their qualified identities. See [non-portable R6 classes](r-package-dev.md#non-portable-r6-classes).
...
For `[`, base subscripting is standard-eval, so indices are checked unless the indexed object is data.table-like: a known `data.table()` / `as.data.table()` / `fread()` object, or an unresolved object when a **bracket-NSE package** is detectably in play (a `library(data.table)` call, a `data.table::` reference, or a package `Imports:`/`importFrom`). data.table is the built-in bracket-NSE package; an installed package whose own container uses the same notation joins the set by shipping a `[subset]` table in its [`inst/raven/nse.toml`](non-standard-evaluation.md#declaring-datatable-style--semantics), optionally naming the constructors (`x <- dibble(...)`) and by-reference converters that classify an object the way `data.table()` and `setDT()` do. Such a package is put in play by the same signals as data.table — its `library()` call, `Imports:`/`importFrom`, or any `pkg::` reference in the file (e.g. `pkg::dibble(...)`). Independently of that classification, a `[` call any of whose arguments is a `:=` assignment (`d[, avg := mean(x), by = grp]`, `d[cond, tag := 0]`, or the functional spelling `` d[, `:=`(a = 1, b = 2)] ``) is always treated as NSE and all of its indices are suppressed: base R has no `:=` function and no base `[` method evaluates one, so Raven treats the call as data.table-style NSE for analysis regardless of which package supplies the method (other packages adopt this notation for their own containers). The `:=` must be the argument itself, not nested inside another expression, and it wins even over a known non-data.table object — `df[, y := 1]` on a plain data.frame is a runtime error either way, and an undefined-variable report on `y` would be misleading. `[[` is always checked — `DT[[x]]` references `x` as a real variable.

data.table's by-reference converters are also recognized: a statement-level `setDT(x)` flips `x` to a data.table from that line onward (so a later `x[, mean(value), by = grp]` no longer flags `value`/`grp`), `setDF(x)` flips it back to a plain data.frame, and `setattr(x, "class", ...)` sets the class explicitly. The transition is positional — a `[` *above* the converter keeps the object's prior classification.
