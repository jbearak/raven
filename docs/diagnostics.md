...
- Tidy-eval **wrapper functions** propagate the data-mask context one level: a local function that embraces a formal into a call argument (`my_filter <- function(data, cond) filter(data, {{ cond }})`) is itself data-masking, so `my_filter(df, x > 2)` does not flag `x` (while `df` stays checked). The same applies to a body that defuses its `...` through a plural capture helper (`enquos(...)`, `quos(...)`, …) or forwards `...` directly into a data-mask position of a covered verb (`function(...) join_by(...)`). The default expression of a captured formal (e.g. a `values_from = value` pivot-style signature) is quoted rather than evaluated, so it is exempt too. Propagation requires this verified forwarding shape — arguments of arbitrary unknown functions are not exempted.
