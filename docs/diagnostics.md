...
**Uncatalogued NSE helpers:** When a function is not in Raven's policy table, use [`# raven: nse`](directives.md#nse-declarations) to declare its argument-evaluation policy. This is preferable to a blanket ignore because it records a reusable per-function policy — whole-call or per-formal — and keeps checking the non-captured arguments. It is position-aware (applies to calls after the directive line), and the most recent declaration for a function wins. When Raven reports an `undefined-variable` inside a call argument whose callee is not a high-confidence standard-eval function (a builtin or base-package export) and is not already governed by an `# raven: nse` directive, the diagnostic message suggests the appropriate `# raven: nse` form, and a quick-fix code action can insert the directive above the call.
