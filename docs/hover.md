...
REditorSupport's hover doesn't resolve scope at the cursor. When a name exists in multiple installed packages, it can list them all. Hovering `filter` after `library(dplyr)`, for example, may show **both** `dplyr::filter` and `stats::filter` as bold links at the top, because REditorSupport falls through to `utils::help((topic))` without a package argument — see [Comparison: Hover help](comparison.md#hover-help) for the mechanism. Raven's single scope-aware result (`dplyr::filter`, in this case) appears underneath.
