...
| `raven.linting.enabled` | `"auto"` | `"auto"` \| `"on"` \| `"off"` \| `true` \| `false` | `linting.enabled` | [linting](linting.md) | Controls native lint diagnostics. "auto" (default) enables linting when a .lintr or raven.toml opts in. true/"on" forces linting on. false/"off" disables linting unless a discovered raven.toml explicitly sets enabled = true (it always overrides .lintr auto-discovery). |
