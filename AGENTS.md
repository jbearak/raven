...
- File-path go-to-definition for AST-detected `source()` calls must use the same workspace-root fallback rules as dependency-graph and scope resolution: fallback is allowed only when there is no explicit or inherited working directory.
