...
- **Knit pipeline (Knit Preview + Pandoc Export)**
  ...
  - The R subprocess calls `knitr::knit` directly (not `rmarkdown::render`). The expression sets `knitr::opts_knit$set(root.dir = <root or getwd()>, base.dir = <previewDir>)`, sets `knitr::opts_chunk$set(fig.path = 'figure/')` plus any YAML-derived chunk options, runs `knitr::knit(input = ..., output = ..., envir = new.env(), quiet = TRUE)`, and emits `cat('Output created: ', out, '\n', sep = '')` so the existing `parseRenderedOutputPath` regex keeps working. Chunk-option values come from R-side variables, never interpolated into the R code string; `dev` is validated against an allowlist before injection.
