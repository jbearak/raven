...
- When multiple events in a sorted timeline need to look up preceding events of a specific type, pre-collect the target event type into a separate sorted Vec and use `partition_point` for O(log n) prefix lookup instead of O(n) re-scan.
