...
- Tree-sitter wraps incomplete expressions inside blocks (e.g., `x <-` inside `if (TRUE) { }`) in a single ERROR node spanning the entire block. Minimize multi-line ERROR ranges to the first line and skip recursion into ERROR children to avoid duplicate diagnostics from nested ERROR nodes.
