...
- **Knit pipeline (Knit Preview + Pandoc Export)**
  ...
  - `pandoc_args:` from YAML is NOT honored — a document could otherwise inject `--output`, `--lua-filter`, `--metadata-file`. Adding support later requires an explicit allowlist/blocklist defined adjacent to `buildPandocArgs`. CSS paths from YAML are resolved against the workspace folder (or `.Rmd` parent if no workspace) and rejected if they escape — see `isUnderContainmentRoot`.
