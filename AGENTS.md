...
- **Module declarations**: a new top-level module on the `raven` crate must be declared in BOTH `crates/raven/src/lib.rs` AND `crates/raven/src/main.rs` — the lib and bin targets have separate module trees.
