...
- **Path resolution**
  ...
  - **Case-leniency is decoupled from the workspace-root fallback** (issue #535). The single-case-insensitive-match leniency (fold a wrong-cased path to the unique on-disk match; exact wins; 2+ ambiguous stays unresolved; ASCII-only) applies in **both** directions — forward and backward. The workspace-root fallback (below) is the only forward-only piece. In `resolve_path_rich`, the `try_workspace_fallback` bool now gates **only** the workspace-root fallback; the leniency branches are unconditional.
...
- **Cross-file `# raven: nse` / `# raven: func` propagation**
  - `# raven: nse` declarations (and the `# raven: func` formal order they consume) propagate over the source graph using the **revalidation-consistent set** `S(Q) = ancestors(Q) ∪ descendants(ancestors(Q) ∪ {Q})`, computed over the snapshot's trimmed neighborhood subgraph (`collect_cross_file_nse` in `handlers.rs`). Both `collect_cross_file_nse` and `compute_affected_dependents_after_edit` build this set from the single shared primitive `DependencyGraph::revalidation_consistent_set` (`cross_file/dependency.rs`), so they use the identical traversal shape (same two graph primitives, chained the same way) and cannot drift in edge-selection logic. The full directed-inverse equivalence additionally depends on inputs the helper does not encode: both callers passing matching `max_depth` / `max_visited` budgets, and the deliberate graph asymmetry — collection over the trimmed neighborhood subgraph, revalidation over the full graph. That asymmetry is intentional and safe-direction (`S_trimmed ⊆ S_full`): collection can only ever omit a foreign suppression (leaving a real diagnostic — a false positive at worst), never fabricate one or drop a needed revalidation; under budget truncation the two graphs can reach different sets, the same safe direction. Each caller keeps its own root handling (collection drops self, sorts, dedups; revalidation subtracts `{Q}` and dedups via a shared `seen` set).
