...
- **Knit pipeline (HTML-only)**
  ...
  - `KnitOutputPanel.activeThemeId` resolves the user's currently-active theme id, NOT just `workbench.colorTheme`. When `window.autoDetectColorScheme` is true, VS Code swaps the active theme between `workbench.preferredLightColorTheme` and `workbench.preferredDarkColorTheme` based on the OS appearance — but `workbench.colorTheme` keeps the user's last manual pick. Reading only that setting gives the WRONG theme for any auto-detect user (e.g. dark-mode preference Dark+ while macOS light mode shows Solarized Light: reading `workbench.colorTheme` yields Dark+, not Solarized Light). The kind from `activeColorTheme.kind` is the source of truth for "which variant is live"; combine it with the preferred-light/dark setting under auto-detect.
