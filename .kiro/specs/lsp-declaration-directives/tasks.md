...
- [ ] 2. Implement directive parsing for declaration directives
  - [ ] 2.1 Add regex patterns for variable declaration directives in `directive.rs`
  ...
  - [ ] 2.2 Add regex patterns for function declaration directives in `directive.rs`
...
- [ ] 4. Integrate declared symbols into scope resolution
  ...
  - [ ] 4.2 Update `compute_artifacts()` to create Declaration events from metadata
...
- [ ] 5. Update interface hash computation
  - [ ] 5.1 Include declared symbols in `compute_interface_hash()` in `scope.rs`
...
- [ ] 7. Implement diagnostic suppression for declared symbols
  - [ ] 7.1 Update undefined variable diagnostic collection in `handlers.rs`
...
- [ ] 8. Implement completion support for declared symbols
  - [ ] 8.1 Update completion handler in `handlers.rs` to include declared symbols
...
- [ ] 9. Implement hover support for declared symbols
  - [ ] 9.1 Update hover handler in `handlers.rs` for declared symbols
...
- [ ] 10. Implement go-to-definition for declared symbols
  - [ ] 10.1 Update go-to-definition handler in `handlers.rs` for declared symbols
...
- [ ] 12. Implement cross-file declaration inheritance
  - [ ] 12.1 Update cross-file scope traversal in `scope.rs` for declared symbols
...
- [ ] 13. Handle conflicting declaration kinds
  - [ ] 13.1 Update scope resolution to handle same symbol declared as both variable and function
...
- [ ] 14. Implement workspace index integration for declarations
  - [ ] 14.1 Update workspace indexer to extract declared symbols from indexed files
    - Extract declarations when indexing closed files
    - Store in workspace index alongside other metadata
    - _Requirements: 12.1_

  - [ ] 14.2 Update scope resolution to include declarations from indexed files
