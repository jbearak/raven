...
**Other editors:** Download a pre-built binary from the [releases page](https://github.com/jbearak/raven/releases), then run `raven --stdio` and connect via your editor's LSP client. Release archives, VSIX installs, and package-manager builds include Raven's Tier 3 sidecar (`names.db`) next to the executable. Source installs through `cargo install --git https://github.com/jbearak/raven raven` install only the executable; base-7 package coverage is embedded in the binary, and `raven packages update` adds the `names.db` sidecar for broad CRAN/Bioconductor coverage. See [Editor Integrations](docs/editor-integrations.md) for Zed, Neovim, and AI agent configurations.

**Build from source:** Raw Cargo/source builds have embedded base-7 R platform coverage, but they do not include the broad Tier 3 CRAN/Bioconductor sidecar unless you install it with `raven packages update` or place `names.db` next to the executable. See [Development Notes](docs/development.md).
