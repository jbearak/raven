...
**Other editors:** Download a pre-built binary from the [releases page](https://github.com/jbearak/raven/releases), then run `raven --stdio` and connect via your editor's LSP client. Release archives and package-manager builds include Raven's Tier 3 sidecar (`names.db`) next to the executable. Source installs through `cargo install --git https://github.com/jbearak/raven raven` ship only the executable; R's base-package coverage is embedded in the binary, and `raven packages update` adds the `names.db` sidecar for broad CRAN/Bioconductor coverage. See [Editor Integrations](docs/editor-integrations.md) for Zed, Neovim, and AI agent configurations.
